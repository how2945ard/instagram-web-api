...
class Instagram {
  constructor(
    { username, password, jar, cookieStore },
    ...
    jar = jar || request.jar(cookieStore)
  ...
  async getHome() {
    return this.request('/?__a=1').then(data => _.get(data, 'graphql.user'))
  }
  ...
  async getUserByUsername({ username }) {
    ...
    }).then(data => _.get(data, 'graphql.user'))
  }
  ...
  async getStoryReelFeed({ onlyStories = false } = {}) {
    return this.request('/graphql/query/', {
      ...
      .then(data => _.get(data, 'data.user.feed_reels_tray.edge_reels_tray_to_reel'))
      .then(edge_reels_tray_to_reel => _.get(edge_reels_tray_to_reel, 'edges'))
      .then(edges => edges.map(edge => _.get(edge, 'node')))
  }
  ...
  async getStoryReels({
    ...
    }).then(data => _.get(data, 'data.reels_media'))
  }
  ...
  async getSimilarUsersWithQueryHash({ privateInstagramID } = {}) {
    return this.request('/graphql/query/', {
      qs: {
        query_hash: 'aec5501414615eca36a9acf075655b1e',
        variables: JSON.stringify({
          user_id: privateInstagramID,
  ...
  async getSimilarUsersWithQueryID({ privateInstagramID } = {}) {
    return this.request('/graphql/query/', {
      qs: {
        query_id: '17845312237175864',
        variables: JSON.stringify({
          id: privateInstagramID,
        })
      }
    }).then(data => _.get(data, 'data'))
  }
  ...
  async getActivity() {
    return this.request('/accounts/activity/?__a=1').then(
      data => _.get(data, 'graphql.user')
    )
  }

  async getProfile() {
    return this.request('/accounts/edit/?__a=1').then(data => _.get(data, 'form_data'))
  }
  ...
  async getMediaByShortcode({ shortcode }) {
    return this.request(`/p/${shortcode}/?__a=1`).then(
      data => _.get(data, 'graphql.shortcode_media')
    )
  }
  ...
  async _getPosts({ userId, perPage = 12, nextPageToken }) {
    ...
    return this.request
      ...
      .then(response => _.get(response, 'data.user'))
  }
}
